...
THEMES = {
    ...
    "night": {
        # Window / toolbar  — deep reds and near-black purples
        "win_bg":        "#0A0005",
        "bar_bg":        "#110008",
        "bar_lbl":       "#6B2230",
        "bar_sep":       "#2A0A10",
...
# ═════════════════════════════════════════════════════════════════════════════
# Yale Bright Star Catalog (BSC5) – embedded subset
# ═════════════════════════════════════════════════════════════════════════════
...
class TitilaApp(tk.Tk):
    REFRESH_MS  = 30_000
    PAN_STEP_PX = 40          # pixels per arrow-key press
    ...
    def __init__(self):
        ...
        # track toolbar widgets that need recoloring on theme switch
    ...
    def _build_ui(self):
        ...
        # Live / time
        chk("Live", self.live_var, self._on_live_toggle).pack(side=tk.LEFT)
        ent(self.date_var, 11).pack(side=tk.LEFT, padx=2)
        ent(self.time_var, 9).pack(side=tk.LEFT, padx=2)
        btn("⟳ Go", self._refresh).pack(side=tk.LEFT, padx=4)
        sep()

        # Sky toggles
        chk("Grid",    self.show_grid,    self._redraw).pack(side=tk.LEFT)
        chk("Labels",  self.show_labels,  self._redraw).pack(side=tk.LEFT)
        chk("Planets", self.show_planets, self._refresh).pack(side=tk.LEFT)
        sep()

        # Reset
        btn("⌖ Reset", self._reset_view, padx=6).pack(side=tk.LEFT)
        sep()

        # Night mode toggle — keep a reference so we can re-style its text
        self._night_chk = chk("🌙 Night", self.night_var, self._toggle_night)
    ...
    _PAD_BTN_SIZE = 28   # px
    _PAD_GAP      = 3    # px between buttons
    _PAD_MARGIN   = 12   # px from canvas edge

    def _pad_layout(self, W, H):
        """
        Return dict of button-name -> (x1, y1, x2, y2) for the control pad
        placed in the bottom-right corner of the canvas.
        Buttons: up, down, left, right, zoom_in, zoom_out, reset
        Layout (3×3 grid, top-right position, missing corners):
          [      ] [ up   ] [      ]
          [ left ] [ rst  ] [ right]
          [zoom- ] [ down ] [zoom+ ]
        """
    ...
    def _toggle_night(self):
        # The BooleanVar is already toggled by the Checkbutton;
        # if called from keyboard, toggle it manually.
    ...
    def _on_canvas_click(self, event):
        """Handle clicks on the control-pad buttons drawn on the canvas."""
        # Only act on short taps (not drag-ends)
    ...
    def _refresh(self):
        ...
        self._status_var.set("Computing positions…")
    ...
    def _on_done(self, objects, t):
        ...
        self._status_var.set(
            ...
            f"{vis} objects above horizon  │  {utc}  │  "
            f"Scroll/+− to zoom  ·  Drag/arrows to pan  ·  0 to reset  ·  N night mode"
        )
        self._redraw()
    ...
    def _draw_hud(self, c, T, W, H):
        ...
        deg100   = 100 / vp.scale
        hud = (f"Centre: Az {az:.1f}°  Alt {alt:.1f}°  │  "
               f"{deg100:.1f}°/100px")
        c.create_text(8, H - 4, text=hud, anchor="sw",
                      fill=T["hud_txt"], font=("Courier", 8))

        # North arrow (top-right corner, avoids control pad)
        n_sx, n_sy = 0.0, -2.0
    ...
    def _draw_control_pad(self, c, T, W, H):
        """Draw the D-pad + zoom + reset buttons in the bottom-right corner."""
        layout = self._pad_layout(W, H)
        ...
        for name, (x1, y1, x2, y2) in layout.items():
            ...
            # Symbol
            label = labels[name]
            # Zoom +/- get slightly different styling
